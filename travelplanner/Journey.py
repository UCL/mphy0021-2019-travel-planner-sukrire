...
class Journey:
    ...
    def plot_bus_load(self):
        ...
        for i, stop in enumerate(self.value):
            ...
            prev = stop
        fig, ax = plt.subplots()
        ax.step(range(len(self.value)),
                list(self.value.values()), where='post')
    ...
    def travel_time(self):
        ...
        self.passid = np.linspace(0, len(self.passenger.passengers),
                                  len(self.passenger.passengers) + 1,
                                  dtype=int)
        ...
        self.traveldic = dict(zip(self.passid, map(list, zip(map(str,
                              list_walktime), map(str, list_bustime)))))
    ...
    def passenger_trip(self, testpass):
        ...
        self.distances = [(math.sqrt((self.route.xcoord[stop] -
                          self.passenger.passengers[testpass][0][0])**2 +
                          (self.route.ycoord[stop] -
                          self.passenger.passengers[testpass][0][1])**2),
                          stop) for stop in self.stops]
        closer_start = min(self.distances)
        #  to endRoute
        self.distances = [(math.sqrt((self.route.xcoord[stop] -
                          self.passenger.passengers[testpass][1][0])**2 +
                          (self.route.ycoord[stop] -
                          self.passenger.passengers[testpass][1][1])**2),
                          stop) for stop in self.stops]
